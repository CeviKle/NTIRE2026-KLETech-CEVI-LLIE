...
class CLIPIQA_Loss(nn.Module):
    ...
    x = torch.nn.functional.interpolate(
        pred, size=(224, 224), mode='bicubic', align_corners=False
    )
    ...
    target_cls = torch.zeros(x.size(0), dtype=torch.long).to(self.device)

    loss = torch.nn.functional.cross_entropy(logits, target_cls)
    loss = self.loss_weight * loss

    return loss, {'clipiqa_loss': loss}